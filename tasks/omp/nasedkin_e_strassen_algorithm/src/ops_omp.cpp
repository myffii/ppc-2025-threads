...
std::vector<double> StrassenOmp::AddMatrices(const std::vector<double> &a, const std::vector<double> &b, int size) {
...
#pragma omp parallel sections
  {
#pragma omp section
    p1 = StrassenMultiply(AddMatrices(a11, a22, half_size), AddMatrices(b11, b22, half_size), half_size, half_size);
#pragma omp section
    p2 = StrassenMultiply(AddMatrices(a21, a22, half_size), b11, half_size, half_size);
#pragma omp section
    p3 = StrassenMultiply(a11, SubtractMatrices(b12, b22, half_size), half_size, half_size);
#pragma omp section
    p4 = StrassenMultiply(a22, SubtractMatrices(b21, b11, half_size), half_size, half_size);
#pragma omp section
    p5 = StrassenMultiply(AddMatrices(a11, a12, half_size), b22, half_size, half_size);
#pragma omp section
    p6 =
        StrassenMultiply(SubtractMatrices(a21, a11, half_size), AddMatrices(b11, b12, half_size), half_size, half_size);
#pragma omp section
    p7 =
        StrassenMultiply(SubtractMatrices(a12, a22, half_size), AddMatrices(b21, b22, half_size), half_size, half_size);
  }
...
std::vector<double> StrassenOmp::PadMatrix(const std::vector<double> &matrix, int original_size, int target_size) {
  std::vector<double> padded_matrix(target_size * target_size, 0);
  for (int i = 0; i < original_size; ++i) {
    std::ranges::copy(matrix.begin() + i * original_size,
                      matrix.begin() + i * original_size + std::min(original_size, target_size),
                      padded_matrix.begin() + i * target_size);
  }
  return padded_matrix;
}

std::vector<double> StrassenOmp::TrimMatrix(const std::vector<double> &matrix, int target_size) {
  std::vector<double> trimmed_matrix(target_size * target_size);
  for (int i = 0; i < target_size; ++i) {
    std::ranges::copy(matrix.begin() + i * target_size, matrix.begin() + i * target_size + target_size,
